...
__all__ = ['get_image_path', 'get_hash', 'MAX_IMAGES_PER_PRODUCT', 'GetProcessRowFunc', 'process_df_helper',
           'process_df', 'contains_multiple_values_in_column', 'appears_in_multiple_categories',
...
#     chunks = np.array_split(df, int(len(df/1000)))
    with Pool(n_workers) as ex:
        return pd.concat(list(ex.imap(proc_func, chunks)))
